...
class DataTableViewController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        RequestManager.getDataFromLocalDataBase()
...
//        }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case 0:
            cell.textLabel?.text = LocalDataManager.locationName
            cell.detailTextLabel?.text = ""
        case 1:
		    let weatherConditions: String
			let weatherConditionsText: String
			
            switch RequestManager.weatherData?.current_weather?.weathercode {
			case 0:
        ...
        case 2:
			...
			if RequestManager.loadingState == .loading {
				...
				switch indexPath.row {
				case 0:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.current_weather?.temperature.description ?? "")"
				case 1:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.hourly?.relativehumidity_2m [0].description ?? "") \(RequestManager.weatherData?.hourly_units?.relativehumidity_2m ?? "")"
				case 2:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.hourly?.surface_pressure[0].description ?? "") \(RequestManager.weatherData?.hourly_units?.surface_pressure ?? "")"
				case 3:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.current_weather?.winddirection ?? 0.0)"
				case 4:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.current_weather?.windspeed ?? 0.0)"
				case 5:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.hourly?.visibility[0].description ?? "") \(RequestManager.weatherData?.hourly_units?.visibility ?? "")"
				case 6:
                    cell.detailTextLabel?.text = "\(RequestManager.weatherData?.current_weather?.time ?? "")"
				default:
					cell.detailTextLabel?.text = "Loading ..."
				}
			}	
